...
|---|---|---|---|---|
| 1 | Coverage (the sample set) and ordering (the acquisition sequence) must be shown to be mathematically separable observables. | computational | Reconstruction of the static spatial field is invariant to acquisition-order permutation while the ordered record itself changes. | PASSED — V2.1 synthetic-field test: ordered-record relative difference ≈ 1.46 with unchanged static spatial reconstruction (\(indian-philosophy-modern-physics/V2/RESEARCH_{GRAPH}.md\), section 5). |
| 2 | Any candidate time-like quantity must survive reparameterization of the traversal path. | theoretical/computational | A monotonic relabeling of the traversal parameter (e.g. `u → u³`) must leave the candidate invariant numerically unchanged. | PASSED (for relational separation) — V2.4: relabeling produced zero numerical change in Fubini–Study/L2 separations (\(V2/RESEARCH_{GRAPH}.md\), section 8). |
